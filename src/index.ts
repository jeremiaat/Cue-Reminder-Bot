...
//   POST / or /api/webhook -> Telegram webhook updates
...
// DB init happens in the background with retries — a sleeping Turso DB
// or slow cold start must never block the listener.
let ready = false;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // Vercel rewrites /api/webhook to /, so handle both paths.
